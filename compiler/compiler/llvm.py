...
class LLVMIRGenerator:
    def __init__(self, func_symbols: list[FunctionSymbol], class_symbols: list[Class]):
    ...
    def generate(self) -> str:
        ...
        for func_sym in self.func_symbols:
            self._emit_function(func_sym)
    ...
    def _emit_function(self, func: FunctionSymbol):
    ...
    def _emit_function_impl(self, func: FunctionSymbol, name: str, linkage: str = "", add_self: bool = False):
        params = [f"{self._type_to_ir(param_sym.type)} %{param_sym.name}" for param_sym in func.params]
        if add_self:
            params.insert(0, "ptr %self")

        self._emit(f"define {linkage} {self._type_to_ir(func.return_type)} @{name}({', '.join(params)}) {{")
        self._temp_idx = 0
        returns = self._emit_block(func.node.block)
        if not returns:
            self._emit("\tret void")
        self._emit("}")
    ...
    def _emit_expr(self, expr: _Expression, local: Optional[str] = None) -> str:
        ...
        if isinstance(expr, BoolExpr):
            ...
            if isinstance(call.callee, SymbolExpr):
                ...
                target_symbol = call.callee.target.symbol if isinstance(call.callee.target, SymbolExpr) else None
                if isinstance(target_symbol, ParameterSymbol) and target_symbol.name == "super":
                    # Super method call
                    if self._current_class is None:
                        fatal_error("No class or superclass")

                    super_class = self._current_class.parent.name if self._current_class.parent else "Object"
                    ...
                    # Static method call
                    call = f"call {self._type_to_ir(function.return_type)} @{target_symbol.name}.{call.callee.member}({", ".join(args)})"
                    pass
                else:
