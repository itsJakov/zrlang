...
from lang_ast import ClassDecl, ClassField, FuncDecl, VarStmt, _Statement, IntExpr, SymbolExpr, AllocExpr, CallExpr, \
...
def compile_method(f, method: FuncDecl, cls: ClassDecl, unit_ctx: UnitContext):
...
def compile_cls(f, cls: ClassDecl, unit_ctx: UnitContext):
    ...
    methods: list[FuncDecl] = list(filter(lambda x: isinstance(x, FuncDecl), cls.members))
    if methods:
...
def compile_ir(f, ast: list[_TopLevelDecl]):
